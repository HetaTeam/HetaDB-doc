...
定义 ``graph_db.py`` 工具支持的入库操作模式。

.. code-block:: yaml

   modes: ["graph2milvus", "graph2pg", "chunk2milvus", "chunk2pg"]

- ``graph2milvus``：知识图谱向量 → Milvus
- ``graph2pg``：知识图谱结构 → PostgreSQL
- ``chunk2milvus``：文本块向量 → Milvus
- ``chunk2pg``：文本块内容 → PostgreSQL
